...
def run_one_dataset(
    ...
    rows = []
    selected_by_budget = {}
    max_budget = min(budget, len(workers))
...
def main() -> None:
    ...
    parser.add_argument("--budget", type=int, default=5, help="Maximum or fixed number of selected annotators.")
    parser.add_argument("--fixed_budget", action="store_true", help="If set, final selection uses exactly --budget workers.")
    ...
    if results.empty:
        ...
        final = results[results["is_final"]].copy()
        print(final[["dataset", "budget", "selected_size", "accuracy", "coverage", "surrogate_value"]].to_string(index=False))
